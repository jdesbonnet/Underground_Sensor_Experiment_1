...
mysql --user=$DBUSER --password=$DBPW --host=$DBHOST $DBNAME -e "SELECT DateTime,Rainfall FROM hour where DateTime > $START_TIME ORDER BY DateTime"  | java ConvertTimeFormat - > ../data/nuig_preciptation.dat
